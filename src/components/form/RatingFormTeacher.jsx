...
export default function RatingFormSchool({ busy, initialState, onSubmit, }) {
  ...
  const [trans_sports, setTrans_sports] = useState(false);
  ...
  const handleOnChangeTrans_sports = ({ target }) => {
    document.getElementById("trans_sports").checked = trans_sports;
    setTrans_sports(target.value);
  };
  ...
  if (initialState) {
    ...
    setTrans_sports(initialState.trans_sports);
    ...
    <div>
      <div className="p-5 dark:bg-primary bg-white rounded space-y-3">
        ...
        Woke Alerts:</h1>
            ...
            <Checkbox label="Trans Sports Policy" prop="trans_sports" checked={trans_sports} onChange={handleOnChangeTrans_sports} />
        ...
        <Submit busy={busy} onClick={handleSubmit} value="Rate This Teacher" />
      </div>
    </div>
  );
}
